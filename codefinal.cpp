...
struct Cache{
    ...
    int check(long long int address){
        ...
        for (int i = 0; i<assoc; i++){
            ...
            if (cache[index][i][2] == address/blocksz && cache[index][i][0]==1){
                // cout<<address<<" "<<cache[index][i][0]endl;
                return 1;
            }
        }
        return 0;
    ...
    int update(long long int address,long long int counter,int dirtybit)
    {
        ...
        if (cache[index][i][2] == address/blocksz){
                cache[index][i][3]=counter;
                cache[index][i][1]=(dirtybit | cache[index][i][1]);
                return 1;
            }
        }
        return 0;
    ...
    int invalidate(long long int address,long long int counter,int dirtybit)
    {
        ...
        for (int i = 0; i<assoc; i++){
        if (cache[index][i][2] == address/blocksz){
                int orig_dirty = cache[index][i][1];
                cache[index][i][3]=-1;
                cache[index][i][2]=-1;
                cache[index][i][1]=0;
                cache[index][i][0]=0;
...
int main(int argc, char *argv[]){//int blsize, int l1size, int l1assoc,int l2size,int l2assoc){
    ...
    while(!myfile.eof()){
        ...
        if (action=="r"){
            ...
            // cout<<L1cache.cache[0][0][2]<<"dirty "<<L1cache.cache[0][0][1]<<" "<<L1cache.cache[0][1][2]<<"dirty "<<L1cache.cache[0][1][1]<<endl;
            int f1 = L1cache.check(address); // is data there or not .
            // cout<<address<<"f1"<<f1<<endl;
            ...
            if (f1==0){
                ...
                // cout<<address<<"f2"<<f2<<endl;
                if (f2==0){
                   L2cache.readmiss += 1;
                   ret_write=L1cache.write(address,glcounter,0);
                //    cout<<L1cache.cache[0][0][2]<<"dirty "<<L1cache.cache[0][0][1]<<" "<<L1cache.cache[0][1][2]<<"dirty "<<L1cache.cache[0][1][1]<<endl;
                //    cout<<L2cache.cache[0][0][2]<<"dirty2 "<<L2cache.cache[0][0][1]<<" "<<L2cache.cache[0][1][2]<<"dirty2 "<<L2cache.cache[0][1][1]<<endl;
                   if(ret_write[3]==1)
                   {
                   ...
                   if (L1cache.check(ret_write[1])){
                                x = L1cache.invalidate(ret_write[1],ret_write[2],ret_write[3]);
                                // cout<<"It was all a dream1"<<endl;
                            }
                   if(ret_write[3]==1 || x == 1)
                   {
                        if(x==0){
                        L2cache.writeback+=1;}
                        else{
            ...
            if (f1==0){
                ...
                //    cout<<L1cache.cache[0][0][2]<<"dirty "<<L1cache.cache[0][0][1]<<" "<<L1cache.cache[0][1][2]<<"dirty "<<L1cache.cache[0][1][1]<<endl;
                   ...
                   glcounter++;
                //    cout<<L2cache.cache[0][0][2]<<"dirty "<<L2cache.cache[0][0][1]<<" "<<L2cache.cache[0][1][2]<<"dirty "<<L2cache.cache[0][1][1]<<endl;
                ...
                //    L2cache.writehit+=1;
                   ...
                   L2cache.update(address,glcounter,1); // ----------- changes made here
                //    ret_read=L2cache.read(address,glcounter);
                   ret_write=L1cache.write(address,glcounter,1);
                   if(ret_write[3]==1)
                   {
