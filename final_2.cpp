// assumption that if data is present in L1 then it must be present in L2
// also if any data evicted from L2 that data in L1 should be invalidated
#include<iostream>
#include<vector>
#include<tuple>
#include<sstream>
        ...
        if (action=="r"){
            ...
            if (f1==0){
                ...
                if (f2==0){
                   ...
                   if(ret_write[0]==0)
                   {
                        L1cache.update(ret_write[1],ret_write[2],-1);
                   }
